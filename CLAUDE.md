...
本リポジトリは `tools/publish-public-mirror.ps1` でフィルタ済みスナップショットを GitHub の public ミラーへ publish できる (運用は [docs/runbooks/public-mirror.md](docs/runbooks/public-mirror.md))。除外・置換・禁止パターンは `tools/public-mirror-rules.psd1` に集約 (このファイル自体はミラー非公開)。**新規ファイルに LAN の実 IP・ホスト固有値・credential 類を書くときは、公開対象になることを意識し、必要ならルールに追加する**。
